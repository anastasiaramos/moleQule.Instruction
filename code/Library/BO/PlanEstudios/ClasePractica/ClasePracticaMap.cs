...
{
	[Serializable()]
	public class ClasePracticaMap : ClassMapping<ClasePracticaRecord>
	{	
		public ClasePracticaMap()
		{
			Table("`ClasePractica`");
			Lazy(true);	
			
			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`ClasePractica_OID_seq`" })); map.Column("`OID`"); });
			...
			Property(x => x.Incompatible, map => { map.Column("`INCOMPATIBLE`"); map.NotNullable(false); map.Length(32768); });
