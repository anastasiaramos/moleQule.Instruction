...
{
	[Serializable()]
	public class PlanEstudiosMap : ClassMapping<PlanEstudiosRecord>
	{	
		public PlanEstudiosMap()
		{
			Table("`PlanEstudios`");
			Lazy(true);	
			
			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`PlanEstudios_OID_seq`" })); map.Column("`OID`"); });
